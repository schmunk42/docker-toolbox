...
RUN curl -L https://get.docker.com/builds/Linux/x86_64/docker-1.11.1.tgz > /tmp/docker-1.11.1.tgz && \
    cd /tmp && tar -xzf ./docker-1.11.1.tgz && \
    rm /tmp/docker-1.11.1.tgz && \
    mv /tmp/docker/docker /opt/local/bin/docker-1.11.1 && \
    chmod +x /opt/local/bin/docker-1.11.1
...
RUN ln -s /opt/local/bin/docker-1.11.1 /opt/local/bin/docker
